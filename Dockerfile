...
RUN curl -sSL https://github.com/wkhtmltopdf/wkhtmltopdf/releases/download/0.12.5/wkhtmltox_0.12.5-1.$(lsb_release -c -s)_amd64.deb -o /tmp/wkhtml.deb \
    ...
    && apt-get install -yq -f --no-install-recommends \
    && rm /tmp/wkhtml.deb \
    && rm -rf /var/lib/apt/lists/*
...
RUN curl -sSL https://www.postgresql.org/media/keys/ACCC4CF8.asc | apt-key add - \
    ...
    && apt-get update -q \
    && DEBIAN_FRONTEND=noninteractive apt-get install -yq postgresql-client-12 \
    && rm -rf /var/lib/apt/lists/*
...
RUN curl -sSL http://nightly.odoo.com/odoo.key | apt-key add - \
    ...
    && apt-get update -q \
    && apt-get install -yq google-chrome-stable=80.0.3987.116-1 \
    && rm -rf /var/lib/apt/lists/*
